...
@Api(name = "gameendpoint", namespace = @ApiNamespace(ownerDomain = "example.com", ownerName = "example.com", packagePath = "treasurehunt"))
public class GameEndpoint {
	...
	public Game getGame(@Named("id") String id) {
	...
	public Game removeGame(@Named("id") String id) {
		EntityManager mgr = getEntityManager();
		Game game = null;
		try {
			game = mgr.find(Game.class, id);
	...
	private boolean containsGame(Game game) {
		...
		try {
			Game item = mgr.find(Game.class, game.getId());
			if (item == null) {
				contains = false;
			}
		} finally {
